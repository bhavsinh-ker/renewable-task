( function( $ ) {
    ...
    $(window).on("click", function(){
        if($('.renewable-mobile-menu').hasClass('active')) {
            $(".renewable-mobile-menu, .renewable-mobile-menu ul").removeClass("active");
            return false;
        }
    });

    // prevent mobile menu close when user will click inside the mobile menu
    $('.renewable-mobile-menu').on("click", function(e){
        e.stopPropagation();
    });

    // to open main mobile menu
    $(".renewable-mobile-menu-btn").on("click", function(e){
        e.preventDefault();
        $(".renewable-mobile-menu").toggleClass("active");
        e.stopPropagation(); // prevent mobile menu close when user will click inside the mobile menu
    });

    // to close main mobile menu
    $(".renewable-mobile-menu-close").on("click", function(e){
        e.preventDefault();
        $(".renewable-mobile-menu").removeClass("active");
    });

    // to open all sub mobile menu items
    $(".renewable-mobile-menu ul li a").on("click", function(e){
    ...
    $(".renewable-mobile-menu ul li").each(function(){
        let subMenuItems = $(this).find(">ul.sub-menu");
        if(subMenuItems.length > 0) {
            $(subMenuItems).prepend('<li><a href="#" class="renewable-mobile-menu-sub-menu-close"><i class="bi bi-chevron-left"></i></a></li>');
        }
    });

    // to back mobile sub menu
    $(".renewable-mobile-menu-sub-menu-close").on("click", function(e){
        e.preventDefault();
